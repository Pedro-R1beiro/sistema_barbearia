...
Para permitir que aplicações frontend hospedadas em domínios diferentes consumam esta API, o CORS (Cross-Origin Resource Sharing) foi habilitado. A configuração atual permite requisições de qualquer origem (**Access-Control-Allow-Origin: \***) e aceita os métodos HTTP **GET**, **POST**, **PATCH**, **DELETE** e **OPTIONS**.
Para testar o CORS, basta acessar qualquer rota dentro da API (Até mesmo uma que não exista) usando o método **OPTIONS**.
